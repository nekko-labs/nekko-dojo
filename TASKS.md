...
| `/community` | Discover real projects + tools | Featured projects (Nekkos, Nekko OSS; README-sourced cards), filter by type/location, Helpful tools (Vaizer-fed skills), Discord CTA |
...
- [x] **T40**: Kotrain rebranded to **Nekkos** (repo `nekko-labs/nekkos`, site nekkos.app; nekkos.dev redirects) and the directory follows: `communities.ts` entry renamed (id, name, GitHub, website, art), `public/projects/kotrain.webp` → `nekkos.webp` (still the pre-rename site capture; recapture once the nekkos.app redesign is live), and every live mention updated in TrainTogether, the community page copy, the *Get Involved in the Community* guide chapter, the spec-driven-development article, README, and the spec. Older shipped notes keep the historical names on purpose. · [spec](SPEC.md#community--projects) · Done: 2026-08-03
- [x] **T39**: Community directory drops the Nekko Dojo Skills card, since that repo is gone. The skills marketplace folded into `nekko-labs/vaizer` on 2026-08-02 (the Vaizer repo now ships the plugins its catalog lists) and `nekko-labs/nekko-dojo-skills` was deleted, so the card's GitHub link would 404 and its website already pointed at vaizer.app/skills. Removed the entry from `src/data/communities.ts` and its orphaned `public/projects/nekko-dojo-skills.webp`, and moved what the card was actually for onto the **Vaizer** card: its description now says the repo is the marketplace it lists, so adding a skill is a low-barrier first open-source contribution, and it gained the `Claude skills` tag so the skills-minded filter still lands somewhere. The Community "Helpful tools" section is untouched: it reads Vaizer's `/api/skills`, which followed the move. · [spec](SPEC.md)
