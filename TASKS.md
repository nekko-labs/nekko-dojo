...
- [x] **T40**: Kotrain rebranded to **Kotrain** (repo `nekko-labs/kotrain`, site kotrain.com; kotrain.com redirects) and the directory follows: `communities.ts` entry renamed (id, name, GitHub, website, art), `public/projects/kotrain.webp` → `kotrain.webp` (still the pre-rename site capture; recapture once the kotrain.com redesign is live), and every live mention updated in TrainTogether, the community page copy, the *Get Involved in the Community* guide chapter, the spec-driven-development article, README, and the spec. Older shipped notes keep the historical names on purpose. · [spec](SPEC.md#community--projects) · Done: 2026-08-03
