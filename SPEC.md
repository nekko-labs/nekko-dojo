...
- **The hub is a chooser, not a list.** `/courses` asks one question, **"What do you want to learn?"**, and answers it with the two paths side by side: *Become an expert in coding* (the classic way, The Guide) and *Learn the new Agentic Coding way* (the agentic way, Applied AI Engineer). *Why:* a visitor arrives with an intent, not a product name, so the page should read as a fork in the road rather than two products stacked down the page. Each panel leads with the outcome, then names the course, who it is for, and the belt the path starts at. The panels carry **no borders** (a recorded STYLESEED 3.4 deviation): the mascot, a per-course glow token (amber for the classic path, violet for the agentic one) and the surface wash carry them instead. Pointing at or keyboard-focusing one panel lifts it, brightens its glow, draws an accent rule under the answer and **dims the other**, so the page feels like a choice being made. A "not sure?" line under the fork nudges beginners to the classic path. `[shipped]`
- **Both courses walk the same path.** The Guide and Applied AI Engineer render their curriculum through one shared `LearningPath` component (scroll-inked dotted rail, emoji stage gates, "New moves unlocked" chips, rank-ups at the milestones, a finish-line stop), and both course pages open with the same header shape (crumb, track eyebrow, outcome headline, mascot, and a `BeltStrip` showing the ranks that path awards before you commit to walking it). *Why:* two courses that visibly share a training method read as one dojo with two routes, and it means the second course's page is already structured for the chapters that will fill it. `[shipped]`
...
- **The curriculum is walked, not listed.** The six stages render through the same `LearningPath` as The Guide, so this course flows through gates, "New moves" and rank-ups exactly like course one. Its ranks are **dan grades** (shodan, nidan, sandan) rather than coloured belts, because this course starts where The Guide ends, at black belt, and there is no colour left to award: you grade up inside the black belt. Until the lessons exist, each stage carries its pitch instead of chapter stops, with one honest "lessons are being written" line above the path, and the path ends on "you build the factory" rather than a final belt. The prerequisite section closes with a full panel pointing at the other path for anyone not ready for this one. `[shipped]`
...
- **Rebrands tracked:** the directory follows its projects when they rename. Nekko MCP became **Hypergate** (hypergate.app) and Nekko Journal became **Getsu**; both entries carry the new name, repo, site and copy, and Kotrain gained its kotrain.com link. Private repos are never listed (Lightwrite, ex Nekko Notes, is deliberately absent), and the guide chapter on getting involved points at Kotrain instead of the retired Nekko Notes. `[shipped]`
